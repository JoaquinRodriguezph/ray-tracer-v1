"""
FB vs Traditional - Chandelier Scene Comparison
Uses the trained multi‑scene FB model and the chandelier scene.
"""
...
class EnhancedEncoder(nn.Module):
    ...
    def __init__(self, obs_dim=22, z_dim=64, hidden_dim=512):
        ...
        self.output = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(hidden_dim, z_dim * 2)   # 128
        )
    def forward(self, x):
        x = self.input_proj(x)
...
class EnhancedForwardModel(nn.Module):
    """Forward model with hidden_dim=512, 3 heads, 2 layers"""
    def __init__(self, z_dim=64, action_dim=2, hidden_dim=512, num_heads=3, num_layers=2):
        super().__init__()
...
# ----------------------------------------------------------------------
# Agent that loads the trained checkpoint (multi‑scene final model)
# ----------------------------------------------------------------------
class TrainedFBAgent:
    def __init__(self, model_path, scene_small_lights, camera_position, device='cpu'):
        ...
        # Create models with correct dimensions (z_dim=64)
        ...
        # Pre‑compute light prototype from the scene's small lights
    ...
    def load(self, model_path):
        ...
        if 'fb_learner_state' in checkpoint:
            ...
            # Fallback in case checkpoint uses different keys
    ...
    def _create_observation(self, point, normal, incoming_dir, material=None, color=None, sphere_id=0):
        """Helper to build a 22‑dim observation (matching training)."""
        if material is None:
    ...
    def _compute_light_prototype(self, small_lights, num_samples_per_light=5):
        ...
        for light in small_lights:
            # Approximate incoming direction: from light toward camera
            ...
            for _ in range(num_samples_per_light):
                ...
                # Incoming ray direction (pointing from surface toward camera)
                incoming = to_camera
        ...
        # Normalise to unit length for cosine similarity (optional, but helps)
    ...
    def choose_direction(self, observation, use_mean=True):
        ...
        with torch.no_grad():
            mean, log_var = self.backward_model(current_z_t, target_z_t)
            if use_mean:
                action = mean.squeeze(0).cpu().numpy()
            else:
                # Sample with exploration noise (optional)
                std = torch.exp(0.5 * log_var)
                action = (mean + std * torch.randn_like(mean)).squeeze(0).cpu().numpy()

        return np.clip(action, -1, 1)

# ----------------------------------------------------------------------
# Chandelier scene generator (variation=0 -> base scene)
# ----------------------------------------------------------------------
def generate_chandelier_scene():
    spheres = []
    scene_id = 1000
    ...
    emitive = Material(reflective=0, transparent=0, emitive=1)
    ...
    for i in range(num_lights):
        ...
        r = int(200 + 55 * math.sin(theta))
        g = int(200 + 55 * math.cos(phi))
        b = int(200 + 55 * math.sin(phi + theta))
        r = max(180, min(255, r))
        g = max(180, min(255, g))
        b = max(180, min(255, b))
    ...
    # Decorative glass/mirror spheres
    spheres.append(Sphere(
        id=scene_id + 40,
        centre=Vector(1.5, 3, 7),
        radius=0.6,
        material=glass,
...
# ----------------------------------------------------------------------
# Renderer classes (adapted from fb_vs_traditional_comparison.py)
# They will be instantiated and then we replace their scene and lights.
# ----------------------------------------------------------------------
class TraditionalRenderer:
    ...
    def render(self, width=200, height=150, samples_per_pixel=4, max_bounces=3):
        ...
        self.stats = {k: 0 for k in self.stats}  # reset
...
class WorkingFBRenderer:
    ...
    def trace_ray_fb(self, ray, bounce_count=0, accumulated_color=Colour(0, 0, 0)):
        ...
        # Direct lighting (same as traditional)
        direct_light = Colour(0, 0, 0)
        for light in self.light_sources:
            if light == sphere:
                continue
        ...
        # Indirect bounce – use FB if available
        indirect_light = Colour(0, 0, 0)

        if material.reflective > 0.9:  # treat as mirror (reflectivity > 90%)
            reflect_dir = ray.D.reflectInVector(normal)
            reflect_ray = Ray(point.addVector(normal.scaleByLength(0.001)), reflect_dir)
            ...
            if use_fb:
                ...
                # Traditional cosine-weighted sampling
                r1, r2 = np.random.random(), np.random.random()
                theta = np.arccos(np.sqrt(r1))
                phi = 2 * np.pi * r2
                if abs(normal.z) > 0.9:
...
def find_latest_model():
    # Look for directories like fb_multi_scene_training_* containing fb_multi_scene_final.pth
    ...
    if candidates:
        ...
        return candidates[0]
    # Fallback: maybe the model is in a different location
    return None

def main():
    print("="*80)
    print("FB vs TRADITIONAL - CHANDELIER SCENE COMPARISON")
    print("="*80)
    ...
    # Generate chandelier scene
    print("Generating chandelier scene...")
    scene_spheres = generate_chandelier_scene()
    ...
    # Camera position (eye level, slightly forward)
    camera_pos = Vector(0, 2, 0)

    # Create renderers and inject the chandelier scene
    ...
    samples_per_pixel = 8
    max_bounces = 8   # enough to reach lights
    ...
    out_dir = Path(f"./chandelier_comparison_{timestamp}")
