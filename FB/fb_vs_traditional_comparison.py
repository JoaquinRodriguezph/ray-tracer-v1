"""
FB vs Traditional - WORKING VERSION with proper architecture matching
"""
...
# Create the EXACT architecture that matches your trained model
class EnhancedEncoder(nn.Module):
    """EXACT architecture from your trained model"""
    def __init__(self, obs_dim=22, z_dim=64, hidden_dim=256):
        super().__init__()
        
        # Input projection (matches error: input_proj.0.weight etc.)
        ...
        # Residual blocks (matches error: res_blocks.*)
        ...
        # Attention (matches error: attention.*)
        self.attention = nn.MultiheadAttention(hidden_dim, num_heads=4, batch_first=True)
        
        # Output (matches error: output.*)
...
class EnhancedForwardModel(nn.Module):
    """Forward model matching your trained model"""
    def __init__(self, z_dim=64, action_dim=2, hidden_dim=256, num_heads=2, num_layers=2):
        super().__init__()
...
class EnhancedBackwardModel(nn.Module):
    """Backward model matching your trained model"""
    def __init__(self, z_dim=64, action_dim=2, hidden_dim=128, num_layers=2):
        super().__init__()
...
class WorkingFBAgent:
    """FB agent with EXACT architecture matching your trained model"""
    
    def __init__(self, model_path=None, device='cpu'):
        self.device = device
        
        # Create EXACT architecture
        self.encoder = EnhancedEncoder(obs_dim=22, z_dim=32, hidden_dim=256).to(device)
        self.forward_model = EnhancedForwardModel(z_dim=32, action_dim=2, hidden_dim=256, num_heads=2, num_layers=2).to(device)
        self.backward_model = EnhancedBackwardModel(z_dim=32, action_dim=2, hidden_dim=128, num_layers=2).to(device)
        
        # Load model if provided
        if model_path and Path(model_path).exists():
            self.load(model_path)
        else:
            print("No FB model provided - using random sampling")
    
    def load(self, model_path):
        """Load model weights with proper architecture"""
        try:
            # Add safe globals
            import numpy
            ...
            if 'fb_learner_state' in checkpoint:
                ...
                print("✓ Loaded full FB agent")
            else:
                print("✗ Checkpoint doesn't contain expected keys")
                return False
    ...
    def encode(self, observation):
        ...
        with torch.no_grad():
            ...
            params = self.encoder(obs_tensor)
            z_mean, z_log_var = params.chunk(2, dim=-1)
            # Use mean for deterministic encoding
            return z_mean.squeeze(0).cpu().numpy()
    
    def choose_direction(self, observation):
        """Choose direction based on observation"""
        # Encode current observation
        current_z = self.encode(observation)
        
        # Create a simple target (toward center of scene)
        target_z = current_z.copy()
        target_z[:3] = 0  # Aim toward origin
        
        # Predict action using backward model
        with torch.no_grad():
            current_tensor = torch.FloatTensor(current_z).unsqueeze(0).to(self.device)
            target_tensor = torch.FloatTensor(target_z).unsqueeze(0).to(self.device)
            
            mean, log_var = self.backward_model(current_tensor, target_tensor)
...
# Traditional Renderer (same as before, but simplified)
class TraditionalRenderer:
    def __init__(self):
        ...
        self.small_lights = [s for s in self.light_sources if s.radius < 0.5]
        self.stats = {
...
class WorkingFBRenderer:
    def __init__(self, model_path=None):
        ...
        self.fb_agent = WorkingFBAgent(model_path, device='cpu')
        self.fb_loaded = model_path and Path(model_path).exists()
    ...
    def trace_ray_fb(self, ray, bounce_count=0, accumulated_color=Colour(0, 0, 0)):
        ...
        # Simple lighting (same as traditional)
        direct_light = Colour(0, 0, 0)
        for light in self.light_sources:
            if light == sphere:
                continue
        ...
        if material.reflective > 0:
            ...
            # Diffuse bounce - USE FB IF AVAILABLE
...
def main():
    ...
    print("="*80)
    print("FB vs TRADITIONAL - WORKING COMPARISON")
    print("="*80)
    
    # Find FB model
    model_path = None
    model_files = list(Path(".").glob("fb_your_scene_*/fb_your_scene_final.pth"))
    if model_files:
        model_files.sort(key=lambda x: x.parent.stat().st_mtime, reverse=True)
        model_path = model_files[0]
        print(f"Found FB model: {model_path}")
    else:
        print("Warning: No FB model found")
        model_path = None
    ...
    fb = WorkingFBRenderer(model_path)
    ...
    print(f"\nFB Usage:")
    print(f"  FB directions attempted: {fb.stats['fb_used']:,}")
    print(f"  FB directions successful: {fb.stats['fb_success']:,}")
    ...
    output_dir = Path(f"./working_comparison_{timestamp}")
    ...
    print("RESULTS SAVED")
    print(f"{'='*60}")
    print(f"Images saved to: {output_dir}/comparison.png")
